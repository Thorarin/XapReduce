...
namespace MVeldhuizen.XapReduce.Tests
{
    ...
    public class ProgramTests
    {
        ...
        public void CheckCommandLineOptions_MissingInputFile_ReturnsError()
        {
            ...
            {
                Input = "Input.xap",
                Sources = new[] {"Source1.xap", "Source2.xap"}
            };
            
            int actual = Program.CheckCommandLineOptions(options, fileSystem);
        ...
        public void Main_InputAndTwoSourceFilesThatExist_CallsMinifier()
        {
            ...
            Program.Minifier.WhenForAnyArgs(m => m.ReduceXap(null)).
                Do(ci => { options = (Options)ci.Args()[0]; });

            const string commandLine = "-i Input.xap -s Source.xap Source2.xap";
            int exitCode = Program.Main(commandLine.Split(' '));

            Assert.AreEqual(0, exitCode);
            Assert.IsNotNull(options);
            Assert.AreEqual("Input.xap", options.Input);
        ...
        public void Main_MinifierThrowsException_ShowsExceptionOnConsole()
        {
            ...
            Program.Minifier.WhenForAnyArgs(m => m.ReduceXap(null)).
                Do(ci => { throw new Exception("Unit Test Exception."); });
